...
class TextExtractor(html.parser.HTMLParser):
    ...
    def get_plaintext(self):
        maxLen,maxPath,maxText,maxTextList = 0,tuple(),'',[]
        for path,textList in self.depthText.items():
            ...
            list1 = (maxLen,maxPath,maxText,maxTextList)
            list2 = ((len(text),path,text,textList))
            #normalized1 = list(list(0 if i is None else i for i in x) for x in list1)
            #normalized2 = list(list(0 if i is None else i for i in x) for x in list2)
            maxLen,maxPath,maxText,maxTextList = max(list1, list2)
    ...
    def parse_endtag(self, i):
        ...
        # unreliable.
        try:
            return html.parser.HTMLParser.parse_endtag(self, i)
...
class HTMLParserNoFootNote():
    ...
    def handle_data(self, data):
        ...
        html.parser.handle_data(self, data)
...
def check_robotstxt(url, useCache, cache_dir, userAgent=None):
    scheme, netloc, url_path, query, fragment = urllib.parse.urlsplit(url)
    robotstxt_url = urllib.parse.urlunsplit((scheme, netloc, '/robots.txt', '', ''))
    
    key = generate_key(robotstxt_url.encode('utf-8'))
    ...
    default_useragent = next((v for k, v in urllib.request.OpenerDirector().addheaders if k == "User-agent"))
...
def extractFromURL(url,
    ...
    # Apply filters.
    if filters:
        filter_names = list(map(str.strip, filters.split(',')))
    ...
    if not html:
        return ''
    html = str(html, encoding=encoding, errors='replace')
